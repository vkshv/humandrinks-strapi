...
export default factories.createCoreController('api::merch.merch', ({ strapi }) => ({
  async create(ctx) {
    const maxSortWeightEntry = await strapi.db.query('api::merch.merch').findMany({
      select: ['sort_weight'],
  ...
  async swapSortWeight(ctx) {
    ...
    try {
      const updatePromises = items.map((item) =>
        strapi.db.query('api::merch.merch').update({
          where: { id: item.id },
  ...
  async promoteSortWeight(ctx) {
    ...
    await strapi.db.connection.raw(
      `UPDATE merchs SET sort_weight = sort_weight - 1 WHERE sort_weight > ?`,
      [item.sort_weight]
    );
